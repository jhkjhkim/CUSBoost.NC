...
class CUSBoostNC_Classifier:
    ...
    def fit(self, X_train, y_train, number_of_clusters, percentage_to_choose_from_each_cluster, lambda_):
        self.models = []
        self.alphas = []
            ...
            if err <= 0:
                ...
                if len(self.models) == 0:
                    ...
                    for alpha, tree_ in zip(self.alphas, self.models):
                        ...
                        negative_index = (Prediction_ != P_int_)
                
                        Prediction_[negative_index] = -1 # instance that have been classified as negative(0) becomes -1
    ...
    def predict_proba_samme(self, X):
        ...
        #proba =  np.linspace(proba) #재호 수정
        #proba = np.array(proba).astype(float) #재호 수정
        np.seterr(divide='ignore', invalid='ignore') #수정
        proba = proba / normalizer
